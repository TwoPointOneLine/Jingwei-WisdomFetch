...
export default function ChatInput({ sending, onSend }: ChatInputProps) {
  // G-01：accept 跟随后端白名单，不再硬编码前端自以为支持的格式
  ...
  const handleFile = async (files: FileList | null) => {
    ...
    setUploadMsg(null)
    try {
      const { task_ids: ids, rejected } = await uploadFiles(list)
      // G-01：有文件被拒时明确告知，避免"上传成功却查不到"
      if (rejected.length) {
        const names = rejected.map((r) => r.filename).join('、')
        setUploadMsg({
          ok: false,
          text: `${rejected.length} 个文件被拒绝：${names}。${rejected[0].reason}`,
        })
      } else {
        setUploadMsg({
          ok: true,
