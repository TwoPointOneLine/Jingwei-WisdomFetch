...
export default function ChatPanel({
  ...
  // G-07：记录最近一次 ask 使用的模型，供「重新提问」复用
          ...
          <div className={`msg ${m.role}`} key={i}>
            ...
            <div className="msg-content">
              {m.streaming && !m.content ? (
                ...
                // G-02：后端返回 Markdown（四段式产品回答/表格/列表），此处渲染而非裸文本
              ...
              <FeedbackBar
                ...
                onRegenerate={
                  i > 0 && session.messages[i - 1]?.role === 'user'
                    ? () => ask(session.messages[i - 1]!.content, lastModelRef.current)
                    : undefined
                }
              />
            )}
        ...
        {open &&
          citations.map((c) => (
            <div className={`source-item${c.external ? ' external' : ''}`} key={c.index}>
              ...
              <span className="src-title">{c.title || c.entry_name || c.product_name || c.source_file || '未知来源'}</span>
              ...
              <div className="src-meta">
                {c.institution_name && <span>机构 {c.institution_name}</span>}
                {c.industry && <span>行业 {c.industry}</span>}
                {c.market && <span>市场 {c.market}</span>}
...
function FeedbackBar({
  ...
  return (
    <div className="feedback-bar">
      ...
      {onRegenerate && (
        <button className="fb-btn" onClick={onRegenerate}>
          🔄 重新提问
        </button>
      )}
      {open && (
