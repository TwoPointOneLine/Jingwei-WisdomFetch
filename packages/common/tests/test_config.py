"""配置层测试：环境读取工具与配置单例。"""
from shopkeeper_common.config import infra_config, lm_config, settings
from shopkeeper_common.config.common import env_bool, env_int, env_str


def test_env_str_default():
    assert env_str("SHOPKEEPER_UNKNOWN_KEY_XYZ") == ""
    assert env_str("SHOPKEEPER_UNKNOWN_KEY_XYZ", "abc") == "abc"


def test_env_bool_default():
    assert env_bool("SHOPKEEPER_UNKNOWN_KEY_XYZ") is False
    assert env_bool("SHOPKEEPER_UNKNOWN_KEY_XYZ", True) is True


def test_env_int_default():
    assert env_int("SHOPKEEPER_UNKNOWN_KEY_XYZ") == 0
    assert env_int("SHOPKEEPER_UNKNOWN_KEY_XYZ", 8081) == 8081
