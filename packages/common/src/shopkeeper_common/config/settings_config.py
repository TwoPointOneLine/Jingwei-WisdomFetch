...
"""
from shopkeeper_common.config.common import env_int, env_str


class SettingsConfig:
    # 导入服务
    import_app_name: str = env_str("IMPORT_APP_NAME", "shopkeeper-import")
    import_app_port: int = env_int("IMPORT_APP_PORT", 8081)
    # 查询服务
    query_app_name: str = env_str("QUERY_APP_NAME", "shopkeeper-query")
    query_app_port: int = env_int("QUERY_APP_PORT", 8082)
    # 认证服务
    auth_app_name: str = env_str("AUTH_APP_NAME", "shopkeeper-auth")
    auth_app_port: int = env_int("AUTH_APP_PORT", 8083)
    # 用户服务
    user_app_name: str = env_str("USER_APP_NAME", "shopkeeper-user")
    user_app_port: int = env_int("USER_APP_PORT", 8084)
    # 网关服务
    gateway_app_name: str = env_str("GATEWAY_APP_NAME", "shopkeeper-gateway")
