...
# ── 项目根目录定位 ─────────────────────────────────────────────
# 优先使用环境变量 SHOPKEEPER_ROOT 指定；否则向上查找包含 uv.lock 的目录
# （即仓库根，该目录同时含 pyproject.toml / docker-compose.yml / start.bat）。
# 公共模块作为独立 wheel 安装到 site-packages 时，文件位置上溯会失效，
# 此时必须通过 SHOPKEEPER_ROOT 显式指定根目录。


def _find_project_root() -> Path:
    env_root = os.getenv("SHOPKEEPER_ROOT")
    if env_root:
    ...
    # packages/common/src/shopkeeper_common/config/common.py -> parents[5] 为仓库根
