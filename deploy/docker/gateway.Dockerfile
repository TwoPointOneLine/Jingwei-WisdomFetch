...
# 2) 仅安装目标包的外部依赖（命中缓存层）
RUN uv sync --package shopkeeper-gateway --no-dev --no-install-project

# 3) 拷贝源码并安装项目本身（editable）
COPY packages/common packages/common
COPY services/gateway services/gateway
RUN uv sync --package shopkeeper-gateway --no-dev
...
CMD ["uvicorn", "shopkeeper_gateway.api.gateway_server.main:app", "--host", "0.0.0.0", "--port", "8080"]
