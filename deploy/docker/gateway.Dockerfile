...
# 2) 仅安装目标包的外部依赖（命中缓存层）
RUN uv sync --package jingwei-gateway --no-dev --no-install-project

# 3) 拷贝源码并安装项目本身（editable）
COPY backend/packages/common packages/common
COPY backend/services/gateway services/gateway
RUN uv sync --package jingwei-gateway --no-dev
...
CMD ["uvicorn", "jingwei_gateway.api.gateway_server.main:app", "--host", "0.0.0.0", "--port", "8080"]
