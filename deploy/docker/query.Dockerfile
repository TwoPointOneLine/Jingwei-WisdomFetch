...
RUN uv sync --package shopkeeper-query --no-dev --no-install-project

# 3) 拷贝源码并安装项目本身（editable）
COPY packages/common packages/common
COPY services/query services/query
RUN uv sync --package shopkeeper-query --no-dev
...
CMD ["uvicorn", "shopkeeper_query.api.query_server.main:app", "--host", "0.0.0.0", "--port", "8082"]
