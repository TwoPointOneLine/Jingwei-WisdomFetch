...
# 2) 仅安装目标包的外部依赖（命中缓存层）
RUN uv sync --package shopkeeper-user --no-dev --no-install-project

# 3) 拷贝源码并安装项目本身（editable）
COPY packages/common packages/common
COPY services/user services/user
RUN uv sync --package shopkeeper-user --no-dev
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
    CMD ["python", "-c", "import urllib.request;urllib.request.urlopen('http://127.0.0.1:8084/health', timeout=3)"]
CMD ["uvicorn", "shopkeeper_user.api.user_server.main:app", "--host", "0.0.0.0", "--port", "8084"]
