...
RUN uv sync --package jingwei-knowledge --no-dev --no-install-project

# 3) 拷贝源码并安装项目本身（editable）
COPY backend/packages/common packages/common
COPY backend/services/knowledge services/knowledge
RUN uv sync --package jingwei-knowledge --no-dev
...
CMD ["uvicorn", "jingwei_knowledge.api.import_server.main:app", "--host", "0.0.0.0", "--port", "8081"]
